...
fn test_tell_message_api_comprehensive() {
    run_tell_message_test("comprehensive", || async {
        ...
        let batch_throughput =
            total_bytes as f64 / file_batch_total.as_secs_f64() / (1024.0 * 1024.0);
        ...
        let file_improvement =
            file_single_total.as_nanos() as f64 / file_batch_total.as_nanos() as f64;
        ...
        println!("   conn.tell_batch(&[&data1, &data2]).await?;           // Manual batch");
        println!("   ");
...
fn test_tell_message_high_volume_performance() {
    run_tell_message_test("high-volume", || async {
        ...
        let individual_throughput = (message_count * message_size) as f64
        ...
        assert!(
        speedup > 1.0,
        "Batching currently only needs to beat single sends while the zero-copy batch path is re-done"
    );
        assert!(
