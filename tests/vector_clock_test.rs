use kameo_remote::{VectorClock, ClockOrdering, NodeId, SecretKey};
...
fn test_vector_clock_large_scale_no_truncation() {
    ...
    assert_eq!(clock.len(), 1500, "Vector clock should preserve all 1500 entries");
...
fn test_vector_clock_deterministic_ordering() {
    ...
    assert_ne!(node1 > node2, node2 > node1, "Node IDs should have consistent ordering");
}
...
async fn test_vector_clock_in_actor_location() {
    ...
    assert!(location1.vector_clock.happens_before(&location2.vector_clock));
    ...
    assert!(location2.vector_clock.happens_before(&location1.vector_clock));
}

#[test]
fn test_vector_clock_conflict_resolution() {
    use kameo_remote::{RemoteActorLocation, PeerId};
    use std::net::SocketAddr;
    ...
    // They should be concurrent
    assert!(location1.vector_clock.is_concurrent(&location2.vector_clock));
...
fn test_actor_removed_vector_clock_ordering() {
    use kameo_remote::{VectorClock, ClockOrdering, SecretKey};
    ...
    // Scenario 3: Removal is concurrent
    {
        ...
        assert_eq!(removal_clock.compare(&current_clock), ClockOrdering::Concurrent);
    }
    ...
    // Scenario 4: Complex case - removal after re-add
    {
        ...
        assert_eq!(early_removal_clock.compare(&add_clock), ClockOrdering::Concurrent);
