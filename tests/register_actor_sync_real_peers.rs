...
async fn test_register_actor_sync_real_single_peer() {
    ...
    // Start first node
    let handle1 = GossipRegistryHandle::new(node1_addr, vec![], Some(config.clone())).await
        .expect("Failed to start node1");
    let node1_actual_addr = handle1.registry.bind_addr;
    println!("✅ Node1 started on {}", node1_actual_addr);
    
    // Start second node  
    let handle2 = GossipRegistryHandle::new(node2_addr, vec![], Some(config.clone())).await
    ...
    peer2.connect(&node2_actual_addr).await.expect("Failed to connect to node2");
    
    let peer1_id = kameo_remote::PeerId::new("node1");
    let peer1 = handle2.add_peer(&peer1_id).await;
    peer1.connect(&node1_actual_addr).await.expect("Failed to connect to node1");
    ...
    println!("📝 Registering actor '{}' with sync on node1...", actor_name);
    ...
    match result {
        Ok(Ok(())) => {
            ...
            assert!(node2_lookup.is_some(), "Actor should be findable on node2 via gossip");
        }
        Ok(Err(e)) => {
...
async fn test_register_actor_sync_real_no_peers_vs_with_peers() {
    ...
    let solo_handle = GossipRegistryHandle::new(solo_addr, vec![], Some(config.clone())).await
    ...
    let result_solo = solo_handle.registry.register_actor_sync(
    ...
    assert!(elapsed_solo < Duration::from_millis(50), "Solo registration should be immediate");
    ...
    match result_peer {
        Ok(Ok(())) => {
            ...
            assert!(elapsed_peer >= Duration::from_millis(10), 
                   "Peer registration should take longer than solo (waited for ACK)");
            assert!(elapsed_peer <= Duration::from_millis(3000), 
...
async fn test_register_actor_sync_real_multiple_peers() {
    ...
    for i in 0..4 {
        ...
        let handle = GossipRegistryHandle::new(addr, vec![], Some(config.clone())).await
...
async fn test_register_actor_sync_real_peer_timeout() {
    ...
    // Start two nodes
    let handle1 = GossipRegistryHandle::new("127.0.0.1:0".parse().unwrap(), vec![], Some(config.clone())).await
        .expect("Failed to start node1");
    let node1_addr = handle1.registry.bind_addr;
    
    let handle2 = GossipRegistryHandle::new("127.0.0.1:0".parse().unwrap(), vec![], Some(config.clone())).await
        .expect("Failed to start node2");
    ...
    let peer2_id = kameo_remote::PeerId::new("node2");
    let peer2 = handle1.add_peer(&peer2_id).await;
    peer2.connect(&node2_addr).await.expect("Failed to connect to node2");
    
    let peer1_id = kameo_remote::PeerId::new("node1");
    let peer1 = handle2.add_peer(&peer1_id).await;
    peer1.connect(&node1_addr).await.expect("Failed to connect to node1");
    ...
    let start = std::time::Instant::now();
    let result = handle1.registry.register_actor_sync(
        actor_name.clone(),
        actor_location,
    ...
    assert!(result.is_ok(), "Registration should succeed even on timeout");
    assert!(elapsed >= Duration::from_millis(450), "Should wait roughly full timeout");
    assert!(elapsed <= Duration::from_millis(800), "Should not exceed timeout by much");
    
    // Actor should still be registered locally
    let lookup = handle1.lookup(&actor_name).await;
    assert!(lookup.is_some(), "Actor should be registered despite timeout");
