...
fn test_tell_with_lookup_and_performance_comparison() {
    run_lookup_test("tell-lookup-performance", || async {
        ...
        node1
            ...
            .unwrap();
        println!(
            "   ✅ Node1: registered 'chat_service' at {}",
            node1.registry.bind_addr
        );
        ...
        node2
            ...
            .unwrap();
        println!(
            "   ✅ Node2: registered 'auth_service' at {}",
            node2.registry.bind_addr
        );
        ...
        node3
            ...
            .unwrap();
        println!(
            "   ✅ Node3: registered 'storage_service' at {}",
            node3.registry.bind_addr
        );
        ...
        chat_ref.tell_batch(&chat_messages).await.unwrap();
        auth_ref.tell_batch(&auth_messages).await.unwrap();
        storage_ref.tell_batch(&storage_messages).await.unwrap();
        ...
        let batch_vs_individual =
            individual_total.as_nanos() as f64 / batch_total.as_nanos() as f64;
        let batch_vs_sequential =
            sequential_total.as_nanos() as f64 / batch_total.as_nanos() as f64;
        ...
        chat_ref.tell_batch(&chat_batch_refs).await.unwrap();
        auth_ref.tell_batch(&auth_batch_refs).await.unwrap();
        storage_ref.tell_batch(&storage_batch_refs).await.unwrap();
        ...
        println!(
            "   remote_actor.ask_streaming_bytes(payload, actor_id, type_hash, timeout).await?;"
        );
