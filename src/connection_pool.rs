...
impl LockFreeStreamHandle {
    ...
    pub async fn write_direct_ask_inline(
        ...
        let command = WriteCommand {
            channel_id: self.channel_id,
            data: WritePayload::DirectAskInline { header, payload },
            sequence: sequence as u64,
            permit: Some(permit),
        };
    ...
    /// Send a response, automatically using streaming for large payloads.
    /// This is the recommended method for sending responses as it handles
    /// both small (ring buffer) and large (streaming) responses transparently.
    ///
    /// # Arguments
    /// * `payload` - The response payload bytes
    /// * `correlation_id` - The correlation ID from the original request
    ...
    pub async fn send_response_auto(&self, payload: &[u8], correlation_id: u16) -> Result<()> {
        ...
        if payload.len() > threshold {
            // Large response: use streaming protocol
            self.stream_response(payload, correlation_id).await
        } else {
...
/// Wrapper for a single tell() payload.
pub struct TellMessage<'a> {
    data: &'a [u8],
}

impl<'a> TellMessage<'a> {
    /// Create a single message wrapper
...
impl CorrelationTracker {
    ...
    async fn wait_for_response_no_timeout(&self, correlation_id: u16) -> Result<bytes::Bytes> {
        let slot = (correlation_id as usize) % PENDING_RESPONSES_SIZE;
        let slot_ref = &self.pending[slot];
...
impl ConnectionHandle {
    ...
    /// Send a response with automatic streaming for large payloads.
    ///
    /// This method automatically uses streaming for responses that exceed
    /// the streaming threshold, preventing ring buffer overflow for large replies.
    /// For small responses, it uses the efficient ring buffer path.
    ///
    /// # Arguments
    /// * `correlation_id` - The correlation ID from the original request
    /// * `payload` - The response payload bytes
    ...
    pub async fn ask_streaming_bytes(
        ...
        let start_msg =
            build_stream_header_bytes(MessageType::StreamStart, &start_header, correlation_id);
        if self
            .stream_handle
            .streaming_tx
            .send(StreamingCommand::WriteBytes(start_msg))
        ...
        while offset < total_size {
            ...
            let header_bytes = build_chunk_header_bytes(&data_header, correlation_id, chunk_len);
        ...
        // Send StreamEnd
        let end_msg =
            build_stream_header_bytes(MessageType::StreamEnd, &start_header, correlation_id);
        if self
            .stream_handle
            .streaming_tx
            .send(StreamingCommand::WriteBytes(end_msg))
...
impl ConnectionPool {
    ...
    pub(crate) async fn get_connection_with_node_id(
        ...
        if let (Some(peer_id), Some(registry_arc)) = (
            peer_id_opt,
            registry_weak.as_ref().and_then(|w| w.upgrade()),
        ) {
            ...
            .and_then(|registry| {
                ...
                tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(async { registry.actor_message_handler.lock().await.clone() })
                })
            });
                ...
                Ok(n) => {
                    ...
                    while partial_msg_buf.len() >= 4 {
                        ...
                        if partial_msg_buf.len() >= total_len {
                            ...
                            if !msg_data.is_empty() {
                                ...
                                if let Some(msg_type) = crate::MessageType::from_byte(msg_type_byte)
                                {
                                    if msg_type == crate::MessageType::DirectAsk {
                                        ...
                                        match parse_direct_message_payload(msg_data) {
                                            Ok(actual_payload) => {
                                            ...
                                            Err(DirectPayloadError::PayloadTruncated {
                                                expected,
                                                available,
                                            }) => {
                                                warn!(peer = %peer_addr, expected = expected, actual = available,
                                                      "DirectAsk payload truncated");
                                            }
                                            Err(DirectPayloadError::HeaderTooShort) => {
                                                warn!(peer = %peer_addr, payload_len = msg_data.len(), "DirectAsk header too short");
                                            }
                                        }
                                        ...
                                        partial_msg_buf.drain(..total_len);
                                        continue;
                                    } else if msg_type == crate::MessageType::DirectResponse {
                                        if msg_data.len()
                                            < crate::framing::DIRECT_RESPONSE_HEADER_LEN
                                        {
                                            warn!(peer = %peer_addr, payload_len = msg_data.len(), "DirectResponse header too short");
                                            partial_msg_buf.drain(..total_len);
                                            continue;
                                        }
                                    ...
                                    match msg_type {
                                        crate::MessageType::Ask => {
                                            ...
                                            match deserialize_registry_message(&aligned_payload) {
                                                Ok(mut registry_msg) => {
                                                ...
                                                if payload.len() >= 24 + payload_len {
                                                    ...
                                                    if let Some(ref registry_weak) = registry_weak {
                                                            ...
                                                            registry_weak.upgrade()
                                                        {
                                                            ...
                                                            {
                                                                Ok(Some(reply_payload)) => {
                                                                    // Use send_response_auto which handles streaming for large responses
                                                                        ...
                                                                        response_handle
                                                                    {
                                                                            ...
                                                                            })
                                                                        {
                                                                            ...
                                                                            {
                                                                                if let Err(e) = stream_handle
                                                                                    .send_response_auto(&reply_payload, correlation_id)
                                                                                    .await
                                                                                {
                                                                                    warn!(peer = %peer_addr, error = %e, "Failed to send ActorAsk response");
                                                                                } else {
                                                                                    debug!(peer = %peer_addr, correlation_id = correlation_id, reply_len = reply_payload.len(), "Sent ActorAsk response through connection pool");
                                                                                }
                                                                            } else {
                                                                                warn!(peer = %peer_addr, "Connection has no stream handle");
                                                                            }
                                        ...
                                        crate::MessageType::StreamStart => {
                                            ...
                                            {
                                                if let Some(header) =
                                                    crate::StreamHeader::from_bytes(
                                                        &payload[RESERVED_BYTES..],
                                                    )
                                                {
                                                    // Extract correlation_id for ask_streaming responses
                                                    // correlation_id of 0 means no response needed (tell-style streaming)
                                                    let stream_correlation_id =
                                                        if correlation_id != 0 {
                                                            Some(correlation_id)
                                                        } else {
                                                            None
                                                    ...
                                                    // Initialize stream assembly with correlation tracking
                                                    if let Some(ref registry_weak) = registry_weak {
                                                        if let Some(registry) =
                                                            registry_weak.upgrade()
                                                        {
                            ...
                            if let Some(ref registry_weak) = registry_weak {
                                if let Some(registry) = registry_weak.upgrade() {
                                    if let Ok(msg) = deserialize_registry_message(msg_data) {
